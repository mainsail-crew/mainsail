...
<template>
	<div>
		<v-card @dragleave="dragLeaveUpload" @dragover="dragOverUpload" @drop.prevent.stop="dragDropUpload" class="fileupload-card mb-3">
			...
			<v-card-text>
				<v-row>
					<v-col class="col-12 d-flex align-center">
						<v-text-field :label="$t('Files.Search')" append-icon="mdi-magnify" clearable dense hide-details outlined single-line style="max-width: 300px;" v-model="search"></v-text-field>
						<v-spacer></v-spacer>
						<input @change="uploadFile" multiple ref="fileUpload" style="display: none" type="file" />
						<v-btn :loading="loadings.includes('gcodeUpload')" :title="$t('Files.UploadNewGcode')" @click="clickUploadButton" class="px-2 minwidth-0 ml-3" color="primary">
							<v-icon>mdi-upload</v-icon>
						</v-btn>
						<v-btn :title="$t('Files.CreateNewDirectory')" @click="createDirectory" class="px-2 minwidth-0 ml-3" color="grey darken-3">
							<v-icon>mdi-folder-plus</v-icon>
						</v-btn>
						<v-btn :title="$t('Files.RefreshCurrentDirectory')" @click="refreshFileList" class="px-2 minwidth-0 ml-3" color="grey darken-3">
							<v-icon>mdi-refresh</v-icon>
						</v-btn>
						<v-menu :close-on-content-click="false" :offset-y="true" :title="$t('Files.SetupCurrentList')">
							<template v-slot:activator="{ on, attrs }">
								<v-btn class="px-2 minwidth-0 ml-3" color="grey darken-3" v-bind="attrs" v-on="on">
									<v-icon>mdi-cog</v-icon>
								</v-btn>
							</template>
							<v-list>
								<v-list-item class="minHeight36">
									<v-checkbox :label="$t('Files.HiddenFiles')" class="mt-0" hide-details v-model="showHiddenFiles"></v-checkbox>
								</v-list-item>
								<v-list-item class="minHeight36">
									<v-checkbox :label="$t('Files.PrintedFiles')" class="mt-0" hide-details v-model="showPrintedFiles"></v-checkbox>
								</v-list-item>
								<v-divider></v-divider>
								<v-list-item class="minHeight36" v-bind:key="header.key" v-for="header of configHeaders">
									<v-checkbox :label="header.text" @change="changeMetadataVisible(header.value)" class="mt-0" hide-details v-model="header.visible"></v-checkbox>
								</v-list-item>
							</v-list>
						</v-menu>
					</v-col>
					...
					<v-col class="col-12 py-2 d-flex align-center">
						...
						<template v-if="this.disk_usage !== null">
							<v-tooltip top>
								...
								<span>
									{{ $t('Files.Used') }}: {{ formatFilesize(this.disk_usage.used) }}
									<br />
									{{ $t('Files.Free') }}: {{ formatFilesize(this.disk_usage.free) }}
									<br />
			...
			>
				<template slot="items">
					<td v-bind:key="header.value" v-for="header in filteredHeaders">{{ header.text }}</td>
				</template>
				...
				<template slot="body.prepend" v-if="(currentPath !== 'gcodes')">
					<tr @click="clickRowGoBack" @dragleave="dragLeaveFilelist" @dragover="dragOverFilelist($event, {isDirectory: true, filename: '..'})" @drop.prevent.stop="dragDropFilelist($event, {isDirectory: true, filename: '..'})" class="file-list-cursor">
						<td class="pr-0 text-center" style="width: 32px;">
							<v-icon>mdi-folder-upload</v-icon>
						</td>
						<td :colspan="filteredHeaders.length" class>..</td>
					</tr>
				</template>

				<template #item="{ index, item }">
					<tr :data-name="item.filename" :key="`${index} ${item.filename}`" @click="clickRow(item)" @contextmenu="showContextMenu($event, item)" @drag="dragFile($event, item)" @dragend="dragendFile($event)" @dragleave="dragLeaveFilelist" @dragover="dragOverFilelist($event, item)" @drop.prevent.stop="dragDropFilelist($event, item)" class="file-list-cursor user-select-none" draggable="true" v-longpress:600="(e) => showContextMenu(e, item)">
						<td class="pr-0 text-center" style="width: 32px;">
							...
							<template v-else>
								<template v-if="getSmallThumbnail(item) && getBigThumbnail(item)">
									<v-tooltip content-class="tooltip__content-opacity1" top v-if="!item.isDirectory && getSmallThumbnail(item) && getBigThumbnail(item)">
										<template v-slot:activator="{ on, attrs }">
											<vue-load-image>
												<img :src="getSmallThumbnail(item)" height="32" slot="image" v-bind="attrs" v-on="on" width="32" />
												<v-progress-circular color="primary" indeterminate slot="preloader"></v-progress-circular>
												<v-icon slot="error">mdi-file</v-icon>
											</vue-load-image>
										</template>
										<span>
											<img :src="getBigThumbnail(item)" width="250" />
										</span>
									</v-tooltip>
								</template>
								<template v-else-if="getSmallThumbnail(item)">
									<vue-load-image>
										<img :src="getSmallThumbnail(item)" height="32" slot="image" width="32" />
										<v-progress-circular color="primary" indeterminate slot="preloader"></v-progress-circular>
										<v-icon slot="error">mdi-file</v-icon>
									</vue-load-image>
								</template>
						...
						<td class="text-center">
							<v-tooltip top v-if="getJobStatus(item)">
								<template v-slot:activator="{ on, attrs }">
									<span v-bind="attrs" v-on="on">
										<v-icon :color="getStatusColor(getJobStatus(item))" small>{{ getStatusIcon(getJobStatus(item)) }}</v-icon>
									</span>
								</template>
			...
			<div :style="'visibility: '+dropzone.visibility+'; opacity: '+dropzone.hidden" class="dragzone">
				<div class="textnode">{{ $t('Files.DropFilesToAddGcode')}}</div>
			</div>
		</v-card>
		<v-snackbar :timeout="-1" :value="true" bottom dark fixed right v-model="uploadSnackbar.status">
			<span class="mr-1" v-if="uploadSnackbar.max > 1">({{ uploadSnackbar.number }}/{{ uploadSnackbar.max }})</span>
			...
			<br />
			<v-progress-linear :value="uploadSnackbar.percent" class="mt-2"></v-progress-linear>
			<template v-slot:action="{ attrs }">
				<v-btn @click="cancelUpload" color="red" style="min-width: auto;" text v-bind="attrs">
					<v-icon class="0">mdi-close</v-icon>
				</v-btn>
			</template>
		</v-snackbar>
		<v-dialog :max-width="getThumbnailWidth(dialogPrintFile.item)" v-model="dialogPrintFile.show">
			<v-card>
				<v-img :src="getBigThumbnail(dialogPrintFile.item)" contain v-if="getBigThumbnail(dialogPrintFile.item)"></v-img>
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn @click="dialogPrintFile.show = false" color="red darken-1" text>{{ $t('Files.No')}}</v-btn>
					<v-btn @click="startPrint(dialogPrintFile.item.filename)" color="green darken-1" text>{{$t('Files.Yes')}}</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
		<v-menu :position-x="contextMenu.x" :position-y="contextMenu.y" absolute offset-y v-model="contextMenu.shown">
			<v-list>
				<v-list-item :disabled="['error', 'printing', 'paused'].includes(klipperState)" @click="clickRow(contextMenu.item, true)" v-if="!contextMenu.item.isDirectory">
					<v-icon class="mr-1">mdi-play</v-icon>
					{{ $t('Files.PrintStart')}}
				</v-list-item>
				<v-list-item :disabled="['error', 'printing', 'paused'].includes(printer_state)" @click="preheat" v-if="
		...
		<v-dialog max-width="400" v-model="dialogCreateDirectory.show">
			<v-card>
				<v-card-title class="headline">{{ $t('Files.NewDirectory') }}</v-card-title>
				<v-card-text>
					{{ $t('Files.PleaseEnterANewDirectoryName') }}
					<v-text-field :rules="input_rules" @keypress.enter="createDirectoryAction" label="Name" ref="inputFieldCreateDirectory" required v-model="dialogCreateDirectory.name"></v-text-field>
				</v-card-text>
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn @click="dialogCreateDirectory.show = false" color text>{{ $t('Files.Cancel') }}</v-btn>
					<v-btn @click="createDirectoryAction" color="primary" text>{{ $t('Files.Create') }}</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
		<v-dialog max-width="400" v-model="dialogRenameFile.show">
			<v-card>
				<v-card-title class="headline">{{ $t('Files.RenameFile')}}</v-card-title>
				<v-card-text>
					<v-text-field :label="$t('Files.Name')" ref="inputFieldRenameFile" required v-model="dialogRenameFile.newName"></v-text-field>
				</v-card-text>
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn @click="dialogRenameFile.show = false" color text>{{ $t('Files.Cancel') }}</v-btn>
					<v-btn @click="renameFileAction" color="primary" text>{{ $t('Files.Rename') }}</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
		<v-dialog max-width="400" v-model="dialogRenameDirectory.show">
			<v-card>
				<v-card-title class="headline">{{ $t('Files.RenameDirectory') }}</v-card-title>
				<v-card-text>
					<v-text-field label="Name" ref="inputFieldRenameDirectory" required v-model="dialogRenameDirectory.newName"></v-text-field>
				</v-card-text>
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn @click="dialogRenameDirectory.show = false" color text>{{ $t('Files.Cancel') }}</v-btn>
					<v-btn @click="renameDirectoryAction" color="primary" text>{{ $t('Files.Rename') }}</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
		<v-dialog max-width="400" v-model="dialogDeleteDirectory.show">
			<v-card>
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn @click="dialogDeleteDirectory.show = false" color text>{{ $t('Files.Cancel') }}</v-btn>
					<v-btn @click="deleteDirectoryAction" color="error" text>{{ $t('Files.Delete') }}</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
	</div>
...
export default class PageFiles extends Mixins(BaseMixin) {
	...
	private input_rules = [(value: string) => value.indexOf(' ') === -1 || 'Name contain spaces!'];
		...
		if (this.files !== null) {
			...
			if (!this.showPrintedFiles) {
				this.files = this.files.filter(
					(file) =>
						this.$store.getters['server/history/getPrintStatus']({
	...
	getSmallThumbnail(item: FileStateFile) {
		if (item.thumbnails?.length) {
			const thumbnail = item.thumbnails.find((thumb) => thumb.width >= 32 && thumb.width <= 64 && thumb.height >= 32 && thumb.height <= 64);
	...
	preheat() {
		...
		if ('first_layer_extr_temp' in file && 'first_layer_bed_temp' in file && !['error', 'printing', 'paused'].includes(this.printer_state)) {
			let gcode = '';
