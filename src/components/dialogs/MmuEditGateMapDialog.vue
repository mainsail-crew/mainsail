<template>
  <div>
    <v-dialog v-model="showDialog" width="800" persistent :fullscreen="isMobile">
        <panel :title="$t('Panels.MmuPanel.EditGateMap')"
                ...
                <v-container fluid>
                    ...
                    <v-row align="start">
                        <v-col class="d-flex justify-start align-center no-padding">
                            <mmu-machine :editGateMap="editGateMap"
                                ...
                                <v-col cols="12" md="6" class="d-flex flex-column justify-start align-left no-padding pt-3">
                                    ...
                                    <v-row>
                                        <v-col cols="6" class="pt-5 ps-6">
                                            <v-switch v-model="useSpoolman"
                                                      :label="$t('Panels.MmuPanel.GateMapDialog.Spoolman')"
                                                      :disabled="spoolmanSupport === SPOOLMAN_PULL || spoolmanSupport === SPOOLMAN_OFF"
                                                      hide-details class="short-switch"/>
                                        </v-col>
                                        <v-col cols="6">
                                            <v-text-field v-model="editGateMap[editGateSelected].spoolId"
                                                          type="number"
                                    ...
                                    <v-row>
                                        <v-col cols="12" class="ps-6">
                                            <v-switch v-model="selectedGateStatus"
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            <v-subheader class="speed-slider-subheader ps-6 pe-1">
                                                ...
                                                <v-spacer></v-spacer>
                                                <v-text-field v-model="editGateMap[editGateSelected].speedOverride"
                                                              type="number" suffix="%"
                                            ...
                                            <v-card-text class="pb-0 pe-0 pt-1 d-flex align-center">
                                                <v-slider v-model="editGateMap[editGateSelected].speedOverride"
                                                          :min="10"
                                                          :max="150"
                                                          hide-details>
                                                    <template #prepend>
                                ...
                                <v-col cols="12" md="6" class="d-flex justify-center">
                                    ...
                                    <div v-else>
                                        <div :class="!spoolIdExists ? 'no-spool' : ''">
                                            ...
                                            <div>
                                                <strong>{{ spoolmanRemainingWeight }}</strong><small class="ml-1">/ {{ spoolmanTotalWeight }}</small>
                                            </div>
                                        </div>
                                        <div style="padding-top: 12px">
                                            <v-btn block color="secondary" class="spoolman-btn"
    ...
    <spoolman-change-spool-dialog :showDialog="showSpoolmanSpoolChooserDialog"
...
export default class MmuEditGateMapDialog extends Mixins(BaseMixin, MmuMixin) {
    ...
    get selectedGateStatus(): boolean {
        return this.editGateMap[this.editGateSelected].status === 1 || this.editGateMap[this.editGateSelected].status === 2;
    }
    ...
    get useSpoolman(): boolean {
        return this.editGateMap[this.editGateSelected].spoolId > 0 || this.editGateMap[this.editGateSelected].spoolId === null
    }
    ...
    temperatureRules = [
        (v: string | number) => {
            ...
            return (!isNaN(num) && num >= 100 && num <= 290) ? true : this.$t('Panels.MmuPanel.GateMapDialog.BadTemperature')
        }
    ]
    ...
    private generateMapString(gateMap) {
        ...
        const jsonString = JSON.stringify(mapObject).replace(/\"(\d+)\":/g, '$1: ').replace(/"/g, "'")
        return jsonString
    }
