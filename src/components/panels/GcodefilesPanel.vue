                ...
                <v-row>
                    <v-col class="col-12 py-2 d-flex align-center">
                        <span
                            ...
                            {{ this.currentPath !== 'gcodes' ? '/' + this.currentPath.substring(7) : '/' }}</span
                        >
                        <v-spacer></v-spacer>
                        <template v-if="this.disk_usage !== null">
                            <v-tooltip top>
                                ...
                                <span>
                                    ...
                                    {{ $t('Files.Total') }}:
                                    {{ formatFilesize(this.disk_usage.total) }}
                                </span>
                            </v-tooltip>
                        </template>
                    </v-col>
            ...
            <v-card>
                ...
                <v-card-text class="pb-0">{{
                    $t('Files.DoYouWantToStartFilename', {
                        filename: dialogPrintFile.item.filename,
                    })
                }}</v-card-text>
...
export default class GcodefilesPanel extends Mixins(BaseMixin) {
    ...
    set currentPath(newVal) {
        this.$store.dispatch('gui/saveSettingWithoutUpload', {
    ...
    set hideMetadataColums(newVal) {
        this.$store.dispatch('gui/saveSetting', {
            name: 'view.gcodefiles.hideMetadataColums',
            value: newVal,
        })
    }
    ...
    set showPrintedFiles(newVal) {
        this.$store.dispatch('gui/saveSetting', {
            name: 'view.gcodefiles.showPrintedFiles',
            value: newVal,
        })
    }
    ...
    set countPerPage(newVal) {
        this.$store.dispatch('gui/saveSetting', {
            name: 'view.gcodefiles.countPerPage',
            value: newVal,
        })
    }
            ...
            if (e.dataTransfer.files.length) {
                const files = [...e.dataTransfer.files].filter((file: File) => {
                    ...
                    if (!validGcodeExtensions.includes(format)) {
                        this.$toast.error(
                            this.$t('Files.WrongFileUploaded', {
                                filename: file.name,
                            }).toString()
                        )
                ...
                for (const file of successFiles) {
                    this.$toast.success(
                        this.$t('Files.SuccessfullyUploaded', {
                            filename: file,
                        }).toString()
                    )
                }
    ...
    changeMetadataVisible(name: string) {
        if (this.headers.filter((header) => header.value === name).length) {
            ...
            this.$store.dispatch('gui/setGcodefilesMetadata', {
                name: name,
                value: value,
            })
        }
    ...
    view3D(item: FileStateFile) {
        this.$router.push({
            path: '/viewer',
            query: { filename: this.currentPath + '/' + item.filename },
        })
    }
}
</script>
