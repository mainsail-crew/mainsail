            ...
            >
                <template #no-data>
                    <div class="text-center">
                        {{ $t('Machine.ConfigFilesPanel.Empty') }}
                    </div>
                </template>
...
export default class ConfigFilesPanel extends Mixins(BaseMixin) {
    ...
    get headers() {
        return [
            { text: '', value: '' },
            {
                text: this.$t('Machine.ConfigFilesPanel.Name'),
                value: 'filename',
            },
            {
                text: this.$t('Machine.ConfigFilesPanel.Filesize'),
                value: 'size',
                align: 'right',
            },
            {
                text: this.$t('Machine.ConfigFilesPanel.LastModified'),
                value: 'modified',
                align: 'right',
            },
        ]
    ...
    set countPerPage(newVal) {
        this.$store.dispatch('gui/saveSetting', {
            name: 'view.configfiles.countPerPage',
            value: newVal,
        })
    }
    ...
    set hideBackupFiles(newVal) {
        this.$store.dispatch('gui/saveSetting', {
            name: 'view.configfiles.hideBackupFiles',
            value: newVal,
        })
    }
    ...
    set root(newVal) {
        this.$store.dispatch('gui/saveSettingWithoutUpload', {
            name: 'view.configfiles.rootPath',
            value: newVal,
        })
    }
    ...
    deleteDirectoryAction() {
        ...
        this.$socket.emit(
            ...
            {
                path: this.absolutePath + '/' + this.dialogDeleteDirectory.item.filename,
                force: true,
            },
    ...
    createFileAction() {
        ...
        axios
            ...
            .then(() => {
                this.$toast.success(
                    this.$t('Files.SuccessfullyCreated', {
                        filename: this.dialogCreateFile.name,
                    }).toString()
                )
