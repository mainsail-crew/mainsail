                    ...
                    <v-row v-for="key in Object.keys(editHeater.additionSensors)" v-bind:key="key">
                        <v-col class="col-12 py-1">
                            <v-checkbox
                                ...
                                :label="
                                    $t('Panels.ToolsPanel.ShowNameInList', {
                                        name: key,
                                    })
                                "
                                hide-details
...
export default class ToolsPanel extends Mixins(BaseMixin) {
    ...
    cooldown(): void {
        this.$store.dispatch('server/addEvent', {
            message: this.cooldownGcode,
            type: 'command',
        })
        this.$socket.emit('printer.gcode.script', {
            script: this.cooldownGcode,
        })
    }
    ...
    openHeater(object: PrinterStateHeater | PrinterStateTemperatureFan | PrinterStateSensor): void {
        ...
        this.editHeater.boolPower = this.$store.getters['gui/getDatasetValue']({
            name: object.name,
            type: 'power',
        })
        this.editHeater.boolSpeed = this.$store.getters['gui/getDatasetValue']({
            name: object.name,
            type: 'speed',
        })
    ...
    setVisibleAdditionalSensor(sensor: string): void {
        ...
        this.$store.dispatch('gui/saveSetting', {
            name,
            value: this.editHeater.additionSensors[sensor].bool,
        })
    }
    ...
    setChartColor(value: string | any): void {
        ...
        this.$store.commit('printer/tempHistory/setColor', {
            name: this.editHeater.name,
            value: value,
        })
