...
export default class WebcamPanel extends Mixins(BaseMixin) {
    @Prop({ default: 'dashboard' }) declare visiblePage?: string
    ...
    get currentCamId(): string {
        ...
        let currentCamId = this.$store.state.gui.view.webcam.currentCam[this.visiblePage ?? ''] ?? 'all'
    ...
    set currentCamId(newVal: string) {
        this.$store.dispatch('gui/setCurrentWebcam', { viewport: this.visiblePage, value: newVal })
    }
