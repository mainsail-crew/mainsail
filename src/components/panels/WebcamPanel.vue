        ...
        <v-card-text v-if="webcams.length" class="px-0 py-0 content d-inline-block">
            <v-row>
                <v-col class="pb-0" style="position: relative">
                    ...
                    <template v-else-if="currentCam.service === 'webrtc-mediamtx'">
                        <webcam-webrtc-mediamtx :cam-settings="currentCam" :collapsible="$route.fullPath !== '/cam'" />
                    </template>
                    <template v-else>
...
import WebcamMediaMTX from '@/components/webcams/WebrtcMediaMTX.vue'
...
@Component({
    components: {
        ...
        'webcam-webrtc-mediamtx': WebcamMediaMTX,
