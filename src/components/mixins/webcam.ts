...
export default class WebcamMixin extends Mixins(BaseMixin) {
    convertUrl(baseUrl: string, printerUrl: string | null) {
        ...
        if (baseUrl.startsWith('/webcam')) {
            const ports = [80]
            ports.push(this.$store.state.server.config?.server?.port ?? 7125)
            ports.push(this.$store.state.server.config?.server?.ssl_port ?? 7130)

            if (!ports.includes(this.hostPort)) {
