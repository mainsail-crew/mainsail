...
export default {
	...
	upload({ state, rootState }) {
		let file = new File([JSON.stringify({ state })], 'ressorcemonitor.json');
		...
		axios.post('//' + rootState.socket.hostname + ':' + rootState.socket.port + '/server/files/upload',
			...
			window.console.info("ressorcemonitor.json successfully uploaded")
		}).catch(() => {
			window.console.error("Error save ressorcemonitor.json!")
		});
	},
