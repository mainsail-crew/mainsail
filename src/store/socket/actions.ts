    ...
    onMessage({ commit, dispatch }, payload) {
        switch (payload.method) {
            ...
            case 'notify_cpu_throttled':
                commit('server/setThrottledState', payload.params[0], {
                    root: true,
                })
                break

            case 'notify_filelist_changed':
                dispatch('files/filelist_changed', payload.params[0], {
                    root: true,
                })
                break
            ...
            case 'notify_power_changed':
                commit('server/power/setStatus', payload.params[0], {
                    root: true,
                })
                break
            ...
            case 'notify_update_refreshed':
                commit('server/updateManager/setStatus', payload.params[0], {
                    root: true,
                })
                break

            case 'notify_history_changed':
                dispatch('server/history/getChanged', payload.params[0], {
                    root: true,
                })
                break

            case 'notify_service_state_changed':
                dispatch('server/serviceStateChanged', payload.params[0], {
                    root: true,
                })
                break

            case 'notify_timelapse_event':
                dispatch('server/timelapse/getEvent', payload.params[0], {
                    root: true,
                })
                break

            case 'notify_job_queue_changed':
                dispatch('server/jobQueue/getEvent', payload.params[0], {
                    root: true,
                })
                break
